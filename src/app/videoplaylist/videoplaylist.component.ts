...
export class VideoplaylistComponent implements OnInit {
  ...
  getCommentByFormation()
  {
    this.serviceForm.getAllComment().subscribe(
      (data: PostComment[]) => {this.comment = data;
    ...
    this.serviceForm.getFilesFormation(this.idFormation)
      .subscribe(
        async res => {
          ...
          this.videoUrl = 'data:audio/mp3;base64,'+this.base64Data;
         ...
         // const data = window.URL.createObjectURL(blob);
          const url = URL.createObjectURL(blobF);
...
*/
  ...
  loadAudioClick(){
    let binary= this.convertDataURIToBinary(this.retrieveResonse[0].data);
